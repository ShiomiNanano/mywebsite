...
async function initDB(env) {
  ...
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS articles (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (method === 'GET' && m) {
      ...
      return json({ ...a, content_text: htmlToPlain(a.content), prev_id: i > 0 ? list[i - 1].id : null, next_id: i < list.length - 1 ? list[i + 1].id : null });
    }
