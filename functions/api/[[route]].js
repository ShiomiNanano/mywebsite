...
// 注意：表已在 D1 里建好；settings / chat_usage 表会在第一次使用对话功能时自动创建。
...
async function chatRateLimit(env, userId) {
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS chat_usage (
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (method === 'POST' && path === '/chat') {
      ...
      const reply = await callDeepSeek(env, full);
      return json({ reply });
    }
    ...
    if (method === 'PUT' && path === '/admin/chat-config') {
      const e = needAdmin(); if (e) return e;
      const body = await readBody(request);
      await setSetting(env, 'chat_enabled', body.enabled ? '1' : '0');
      await setSetting(env, 'chat_persona', (body.persona || '').trim());
      await setSetting(env, 'chat_greeting', (body.greeting || '').trim());
      return json({ ok: true });
    }
