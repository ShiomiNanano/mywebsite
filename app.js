...
<svg viewBox="0 0 320 200" class="cover-svg" aria-hidden="true">
  <defs>
    <radialGradient id="moonGlow" cx="50%" cy="50%" r="50%">
      <stop offset="0%" stop-color="rgba(232,195,106,0.35)"/>
      <stop offset="100%" stop-color="rgba(232,195,106,0)"/>
    </radialGradient>
  </defs>
  ...
  <g fill="#e8c36a" opacity="0.75">
    <circle cx="60" cy="40" r="1.6"/><circle cx="120" cy="70" r="1.2"/>
    <circle cx="40" cy="90" r="1.4"/><circle cx="200" cy="112" r="1.2"/>
    <circle cx="286" cy="122" r="1.5"/><circle cx="70" cy="150" r="1.3"/>
  ...
  <g fill="#f5efe2">
    ...
    <path d="M 66 70 L 60 52 L 75 64 Z"/>
    <path d="M 82 66 L 92 50 L 92 70 Z"/>
  ...
  <g fill="none" stroke="#1c1710" stroke-width="2" stroke-linecap="round">
    <path d="M 69 80 Q 74 76 79 80"/>
    <path d="M 84 80 Q 89 76 94 80"/>
  </g>
</svg>`;
...
const App = {
  ...
  async ensureMenu() {
    if (!this.state.menu) this.state.menu = await API.get('/api/menu');
    ...
    <div class="page login-page">
      ...
      <div class="login-wrap">
        <div class="login-card">
          ...
          <div class="demo-tip">
            <p>演示管理员：<b>admin / admin123</b></p>
            <p>演示用户　：<b>demo / demo123</b></p>
          </div>
        </div>
      </div>
    </div>`;
  ...
  async renderMenu() {
    ...
    const chapterCount = m.main.length;
    const sectionCount = m.main.reduce((s, c) => s + c.sections.length, 0);
    return `
    <div class="page">
      ${this.topbar()}
      <div class="menu-hero">
      ...
      <div class="menu-cards">
        <a class="menu-card menu-main" href="#/main">
          ...
          <div class="menu-card-content">
            <h3>主线故事</h3>
            <p>第一卷 · 如暖阳般和煦</p>
            <span class="menu-card-meta">${chapterCount} 章 · ${sectionCount} 节</span>
          </div>
  ...
  async renderRead(id) {
    ...
    <div class="page">
      ...
      <section class="comments">
        <h2 class="comments-title">留言 <span class="comments-count">${comments.length}</span></h2>
  ...
  async renderAdminArticles() {
    ...
    const catName = { main: '主线', personal: '个人章', settings: '设定' };
    return `
    <div class="page">
      ${this.topbar()}
      ...
      <div class="admin-table-wrap">
        <table class="admin-table">
          ...
          ${articles.map(a => `
            <tr>
              <td>${a.id}</td>
              <td><span class="tag tag-${a.category}">${catName[a.category]}</span></td>
      ...
      <div class="admin-form-wrap">
        <form id="adminForm" data-id="${id || ''}" class="admin-form">
          ...
          <div class="form-row">
            ...
            <textarea name="content" rows="18" placeholder="在此粘贴正文……">${a ? esc(a.content_text) : ''}</textarea>
          </div>
          <div class="form-row">
